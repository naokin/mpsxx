...
struct Functor<3,Q> {
  ...
  typedef std::vector<btas::QSTArray<double,4,Q>> MPO_type;

  typedef std::vector<btas::QSTArray<double,3,Q>> HOp_type;
    ...
    const std::vector<MPS_type>& wfn)
  {
    ...
    for(size_t k = 0; k < N_-1; ++k) {
      for(size_t g = 0; g < mpo.size(); ++g) {
        compute_sigma_vector(mpo[g],lHopr[k][g],rHopr[k][g],wfn[k],H_[k]);
      }
  ...
  {
    Communicator world;
    for(size_t g = 0; g < pMPO_->size(); ++g) {
      compute_sigma_vector((*pMPO_)[g],(*pLOp_)[g],(*pROp_)[g],c,v);
    }
    for(size_t k = 0; k < N_-1; ++k) {
...
struct Functor<4,Q> {
    ...
    const std::vector<MPS_type>& wfn)
  {
    ...
    for(size_t k = 0; k < N_-1; ++k) {
      for(size_t g = 0; g < lmpo.size(); ++g) {
        compute_sigma_vector(lmpo[g],rmpo[g],lHopr[k][g],rHopr[k][g],wfn[k],H_[k]);
      }
      compute_sigma_vector(lSopr[k],rSopr[k],wfn[k],S_[k]);
      E_[k] = E[k];
    }
  ...
  {
    Communicator world;
    for(size_t g = 0; g < pLMPO_->size(); ++g) {
      compute_sigma_vector((*pLMPO_)[g],(*pRMPO_)[g],(*pLOp_)[g],(*pROp_)[g],c,v);
    }
    for(size_t k = 0; k < N_-1; ++k) {
      double Hk = btas::Dotc(H_[k],c);
      double Sk = btas::Dotc(S_[k],c);
...
struct Functor<2,Q> {
    ...
    const std::vector<MPS_type>& wfn)
  {
    ...
    for(size_t k = 0; k < N_-1; ++k) {
      for(size_t g = 0; g < lHopr[k].size(); ++g) {
        compute_sigma_vector(lHopr[k][g],rHopr[k][g],wfn[k],H_[k]);
      }
      compute_sigma_vector(lSopr[k],rSopr[k],wfn[k],S_[k]);
      E_[k] = E[k];
    }
  ...
  {
    Communicator world;
    for(size_t g = 0; g < pLOp_->size(); ++g) {
      compute_sigma_vector((*pLOp_)[g],(*pROp_)[g],c,v);
    }
    for(size_t k = 0; k < N_-1; ++k) {
      double Hk = btas::Dotc(H_[k],c);
      double Sk = btas::Dotc(S_[k],c);
