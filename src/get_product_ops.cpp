...
(const BIT_OPERATOR_TYPE& l_op, const BIT_OPERATOR_TYPE& s_op, const std::vector<size_t>& r_indxs, bool swap_sweep_dir)
{
  ...
  if(s_op == IDEN) {
    ...
    if((l_op & NORMAL & TYPE) == DOUBLE) {
      if(swap_sweep_dir) {
        ...
        if(has_diag) {
          BIT_OPERATOR_TYPE r_op_trans = DOUBLE | ((r_op & FIRST) >> INDEX_SHIFT) | ((r_op & SECOND) << INDEX_SHIFT);
          for(size_t i = 0; i < r_indxs.size(); ++i) {
            size_t ix = r_indxs[i];
            ls_ops.push_back(r_op_trans | (ix << INDEX_SHIFT) | ix);
            for(size_t j = 0; j < i; ++j) {
              size_t jx = r_indxs[j];
              if(ix > jx)
                ls_ops.push_back(r_op_trans | (jx << INDEX_SHIFT) | ix);
              else
                ls_ops.push_back(r_op_trans | (ix << INDEX_SHIFT) | jx);
            }
          }
          if(r_op_trans == (l_op & MASK)) {
            r_op       ^= SPIN_D;
            r_op_trans ^= SPIN_D;
            for(size_t i = 0; i < r_indxs.size(); ++i) {
              size_t ix = r_indxs[i];
    ...
    // L = Ci or CiComp -> R = DiComp or Di, etc...
      ...
      BIT_OPERATOR_TYPE conj = swap_sweep_dir ? (CONJ_S | COMP) : ZERO;
      // Create product block operators
      if(l_op & COMP) {
      ...
      // Needs conjugation due to swap sweep direction?
      BIT_OPERATOR_TYPE conj = swap_sweep_dir ? COMP : ZERO;
      ls_ops.push_back(l_op ^ conj);
    }
  }
  // L == Identity
    ...
    // S = CiCiDj -> Rk or S = Ci
    else if((s_op & NORMAL & TYPE) == SINGLE) {
      // Needs conjugation due to swap sweep direction?
      BIT_OPERATOR_TYPE conj = swap_sweep_dir ? (CONJ_S | COMP) : ZERO;
      // Create product block operators
      if(s_op & COMP) {
  ...
  // L = Ci or Rk and S = Cj or Rj
  else if((l_op & NORMAL & TYPE) == SINGLE && (s_op & NORMAL & TYPE) == SINGLE) {
    // Ci(L) x Cj(S) -> CiCj(LS)
    if((l_op & COMP) == 0 && (s_op & COMP) == 0) {
      if(swap_sweep_dir) {
        BIT_OPERATOR_TYPE ls_op = DOUBLE | (l_op & FIRST) | ((s_op & FIRST) >> INDEX_SHIFT);
        // Fix operator type for right block
        BIT_OPERATOR_TYPE r_op = (ls_op ^ CONJ_D) & MASK;
        // Check quantum number to have Pii or Qii component
        bool has_diag = ((r_op & FIRST) >> INDEX_SHIFT) != (r_op & SECOND);
        // Create product block operators
        for(size_t i = 0; i < r_indxs.size(); ++i) {
        ...
        size_t ix = (l_op & FIRST & INDEX) >> INDEX_SHIFT;
        size_t jx = (s_op & FIRST & INDEX) >> INDEX_SHIFT;
        if(ix > jx)
          ls_ops.push_back(DOUBLE | (s_op & FIRST) | (l_op & FIRST) >> INDEX_SHIFT);
        else
          ls_ops.push_back(DOUBLE | (l_op & FIRST) | (s_op & FIRST) >> INDEX_SHIFT);
      }
    }
    // Rj(L) x Cj(S) -> H
    else if((l_op & COMP) && ((l_op ^ CONJ_S) & NORMAL) == s_op) {
      if(swap_sweep_dir)
    ...
    // Ci(L) x Rj(S) -> H
    else if((l_op & COMP) == 0 && ((s_op ^ CONJ_S) & MASK) == (l_op & MASK)) {
      if(swap_sweep_dir)
        ls_ops.push_back(IDEN);
      else
        ls_ops.push_back(HAM);
  ...
  // L = CiCj and S = Dk, etc...
    ...
    if(abs(q_ls.p()) == 1 && abs(q_ls.s()) == 1) {
      // Needs conjugation due to swap sweep direction?
      BIT_OPERATOR_TYPE conj = swap_sweep_dir ? COMP : CONJ_S;
      // Fix operator type for right block
      BIT_OPERATOR_TYPE r_op = (l_op & FIRST) ^ ((l_op & SECOND) << INDEX_SHIFT) ^ (s_op & FIRST);
                        r_op = (COMP | SINGLE | (r_op & MASK & FIRST)) ^ conj;
      // Create product block operators
      for(size_t i = 0; i < r_indxs.size(); ++i) {
        size_t ix = r_indxs[i];
        ls_ops.push_back(r_op | (ix << INDEX_SHIFT));
