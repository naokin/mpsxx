...
void mpsxx::boundary_opinfo::reset(size_t L, size_t N, bool ENABLE_SWAP_SWEEP)
{
  using namespace mpogen::bit_operator;
  size_t R = N-L;
  size_t N_estm;
  std::vector<size_t> loop_indices;
  if(!ENABLE_SWAP_SWEEP || L < R) { /* loop indices = 0...L-1 */
    m_bn_dir = FORWARD;
...
//  for(size_t i = N-1; i >= L; --i) loop_indices.push_back(i);
    // Estimated size of operators
  ...
  size_t nop = 0;
  // I
  if(ENABLE_SWAP_SWEEP || R > 0)
    m_bn_ops.insert(std::make_pair(IDEN, nop++));
  // H
  if((!ENABLE_SWAP_SWEEP || R > 0) && L > 0)
    m_bn_ops.insert(std::make_pair(HAM , nop++));
  if(L == 0 || R == 0) return;

  mpogen::BIT_OPERATOR_TYPE comp_type = (!ENABLE_SWAP_SWEEP || L < R) ? ZERO : COMP;
...
//    if(ix < jx) std::swap(ix, jx);
      ...
      m_bn_ops.insert(std::make_pair((CRE_A_CRE_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((CRE_A_CRE_B | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((CRE_B_CRE_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((CRE_B_CRE_B | (jx << INDEX_SHIFT) | ix), nop++));

      m_bn_ops.insert(std::make_pair((CRE_A_DES_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((CRE_A_DES_B | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((CRE_B_DES_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((CRE_B_DES_B | (jx << INDEX_SHIFT) | ix), nop++));

      m_bn_ops.insert(std::make_pair((DES_A_CRE_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((DES_A_CRE_B | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((DES_B_CRE_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((DES_B_CRE_B | (jx << INDEX_SHIFT) | ix), nop++));

      m_bn_ops.insert(std::make_pair((DES_A_DES_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((DES_A_DES_B | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((DES_B_DES_A | (jx << INDEX_SHIFT) | ix), nop++));
      m_bn_ops.insert(std::make_pair((DES_B_DES_B | (jx << INDEX_SHIFT) | ix), nop++));
    }
  }
