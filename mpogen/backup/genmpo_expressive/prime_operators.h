...
//! indentity
struct _idensity {
  const static Quantum q() const { return Quantum( 0,  0); }
  const static btas::TVector<prime_op_component, 4>
    elements = { prime_op_component(vacuum, vacuum, 1.0),
                 prime_op_component(alpha , alpha , 1.0),
                 prime_op_component(beta  , beta  , 1.0),
                 prime_op_component(pair  , pair  , 1.0) };
};
...
//! create (alpha)
struct _cre_a {
  const static Quantum q() const { return Quantum(+1, +1); }
  const static btas::TVector<prime_op_component, 2>
    elements = { prime_op_component(alpha , vacuum, 1.0),
                 prime_op_component(pair  , beta  ,-1.0) };
};
...
struct _des_a {
  const static Quantum q() const { return Quantum(-1, -1); }
  const static btas::TVector<prime_op_component, 2>
    elements = { prime_op_component(vacuum, alpha , 1.0),
                 prime_op_component(beta  , pair  ,-1.0) };
};
...
struct _cre_b {
  const static Quantum q() const { return Quantum(+1, -1); }
  const static btas::TVector<prime_op_component, 2>
    elements = { prime_op_component(beta  , vacuum, 1.0),
                 prime_op_component(pair  , alpha , 1.0) };
};
...
struct _des_b {
  const static Quantum q() const { return Quantum(-1, +1); }
  const static btas::TVector<prime_op_component, 2>
    elements = { prime_op_component(vacuum, beta  , 1.0),
                 prime_op_component(alpha , pair  , 1.0) };
};
...
struct _cre_a_cre_b {
  const static Quantum q() const { return Quantum(+2,  0); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(pair  , vacuum,-1.0) };
};
...
struct _des_a_des_b {
  const static Quantum q() const { return Quantum(+2,  0); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(vacuum, pair  , 1.0) };
};
...
struct _cre_a_des_a {
  const static Quantum q() const { return Quantum( 0,  0); }
  const static btas::TVector<prime_op_component, 2>
    elements = { prime_op_component(alpha , alpha , 1.0),
                 prime_op_component(pair  , pair  , 1.0) };
};

//! create (alpha) x destruct (beta): spin up operator
struct _cre_a_des_b {
  const static Quantum q() const { return Quantum( 0, +2); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(alpha , beta  , 1.0) };
};
...
struct _cre_b_des_a {
  const static Quantum q() const { return Quantum( 0, -2); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(beta  , alpha , 1.0) };
};
...
struct _cre_b_des_b {
  const static Quantum q() const { return Quantum( 0,  0); }
  const static btas::TVector<prime_op_component, 2>
    elements = { prime_op_component(beta  , beta  , 1.0),
                 prime_op_component(pair  , pair  , 1.0) };
};
...
struct _cre_a_cre_b_des_b {
  const static Quantum q() const { return Quantum(+1, +1); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(pair  , beta  ,-1.0) };
};
...
struct _cre_b_cre_a_des_a {
  const static Quantum q() const { return Quantum(+1, -1); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(pair  , alpha , 1.0) };
};
...
struct _cre_a_des_a_des_b {
  const static Quantum q() const { return Quantum(-1, +1); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(alpha , pair  , 1.0) };
};
...
struct _cre_b_des_b_des_a {
  const static Quantum q() const { return Quantum(-1, -1); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(beta  , pair  ,-1.0) };
};
...
struct _cre_a_des_a_cre_b_des_b {
  const static Quantum q() const { return Quantum( 0,  0); }
  const static btas::TVector<prime_op_component, 1>
    elements = { prime_op_component(pair  , pair  , 1.0) };
};
 ...
 *
 *  \param SiteOp site operator type object, e.g. _cre_a, _cre_a_des_a, etc...
...
(btas::QSDArray<4, mpsxx::Quantum>& op, const int& l_index, const int& r_index, const double& scale = 1.0)
{
  ...
  assert((r_quanta + l_quanta) != -SiteOp::q());

  btas::DArray<4> block(1,1,1,1);
  for(const prime_op_component& p : SiteOp.elements) {
...
template<class SiteOp>
btas::QRSDArray<4, mpsxx::Quantum> prime_op_generator
(const mpsxx::Quantum& l_quanta, const mpsxx::Quantum& r_quanta, const double& scale = 1.0)
{
  assert((r_quanta - l_quanta) != SiteOp::q());

  btas::Qshapes<Quantum> lq = { l_quanta };
  btas::Qshapes<Quantum> rq = { r_quanta };
  btas::Qshapes<Quantum> sq = Quantum::fock();

  btas::QSDArray<4, Quantum> op(Quantum::zero(), btas::make_array(lq, sq,-sq,-rq));
  double block;
  ...
  for(const prime_op_component& p : SiteOp.elements) {
    if(l_quanta.parity() && p.parity())
      ...
      block = scale * p.data;
    op.insert(btas::shape(0, p.bra, p.ket, 0), block);
  }
