...
//                                                                                    ||   |  + 1st index  |  + 2nd index
//                                                                                    ||   |  |            |  |
const BIT_OPERATOR_TYPE  ZERO        = 0x00000000; // 0000 0000 0000 0000 0000 0000 0000 0000 -- zero component
...
const BIT_OPERATOR_TYPE  DOUBLE      = 0x20000000; // 0010 00 000000000000 00 000000000000 -- two index operator mask
const BIT_OPERATOR_TYPE  SINGLE      = 0x10000000; // 0001 00 000000000000 00 000000000000 -- one index operator mask
...
const BIT_OPERATOR_TYPE  SINGLE_1    = 0x20000000; // 0010 00 000000000000 00 000000000000 -- one index operator mask 1
...
const BIT_OPERATOR_TYPE  CRE_1       = 0x28000000; // 0010 10 000000000000 00 000000000000
const BIT_OPERATOR_TYPE  CRE_2       = 0x10002000; // 0001 00 000000000000 10 000000000000
const BIT_OPERATOR_TYPE  DES_1       = 0x20000000; // 0010 00 000000000000 00 000000000000
const BIT_OPERATOR_TYPE  DES_2       = 0x10000000; // 0001 00 000000000000 00 000000000000
...
(const mpsxx::fermionic::BIT_OPERATOR_TYPE& _type)
{
  using namespace bit_operator;
  int p = 0; // particle #
  int s = 0; // spin #
  if(_type & IDEN == 0) { // except for I and H
    if(_type & SINGLE_1) {
      BIT_OPERATOR_TYPE op1 = (_type & MASK & FIRST)  >> INDEX_SHIFT >> FIELD_SHIFT;
      if(op1 & 2) { ++p; if(op1 & 1) ++s; else --s; }
      else        { --p; if(op1 & 1) --s; else ++s; }
    }
    if(_type & SINGLE_2) {
      BIT_OPERATOR_TYPE op2 = (_type & MASK & SECOND) >> FIELD_SHIFT;
...
std::string mpsxx::fermionic::translate_bit_op
(const mpsxx::fermionic::BIT_OPERATOR_TYPE& _type)
{
  using namespace bit_operator;
  std::ostringstream opname;
  if(_type & IDEN) {
    if(_type & COMP) opname << "H";
    else             opname << "I";
  }
  else {
    if(_type & COMP) opname << "R:";
    if(_type & SINGLE_1) {
      BIT_OPERATOR_TYPE op1 = (_type & MASK & FIRST)  >> INDEX_SHIFT >> FIELD_SHIFT;
      if(op1 & 2) opname << "Cre";
      else        opname << "Des";
      ...
      opname << ((_type & INDEX & FIRST) >> INDEX_SHIFT) << "]";
    }
    if(_type & SINGLE_2) {
      BIT_OPERATOR_TYPE op2 = (_type & MASK & SECOND)  >> FIELD_SHIFT;
