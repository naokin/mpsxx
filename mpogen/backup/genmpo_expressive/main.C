      ...
      for(auto l = l_ops.begin(); l != l_ops.end(); ++l) {
        for(auto s = s_ops.begin(); s != s_ops.end(); ++s) {
          ...
          if(r_ops.direction() == boundary_opinfo::FORWARD)
            ls_ops = get_product_ops(l->first, s->first, r_indxs, false);
          else // swapping sweep direction
            ls_ops = get_product_ops(l->first, s->first, r_indxs, true);
          // create operator
          for(size_t j = 0; j < ls_ops.size(); ++j) {
            auto r = r_ops.find(ls_ops[j]);
            if(r != r_ops.end()) {
              std::cout << translate(l->first) << " x "
                        << translate(s->first) << " -> "
                        << translate(r->first) << std::endl;
              ++nnz;
            }
          }
        }
