  ...
  const std::string& prefix)
{
  ...
  for(int i = 0; i < N; ++i) {
    ...
    if(enable_swap_sweep_dir) {
      if     (swap_sweep_dir)
        mpos[i].resize(fermion::zero(), btas::make_array( l_ops.get_qshape(), MpSite<fermion>::quanta(),-MpSite<fermion>::quanta(), r_ops.get_qshape()), btas::make_array(l_dshape, s_dshape, s_dshape, r_dshape), false);
...
//            std::cout << "DEBUG[gen_qc_operators]: " << translate(l->first) << " x " << translate(s->first) << " -> " << translate(r->first) << std::endl;
              gen_site_operator(mpos[i], l->first, l->second, s->first, r->first, r->second, oneint, twoint);
...
//            if(nnz_local %  100 == 0) std::cout << nnz_local / 100 << "..." << std::flush;
//            if(nnz_local % 1000 == 0) std::cout << std::endl << "\t\t                               ..." << std::flush;
//            std::cout << "DEBUG[gen_qc_operators]: " << translate(l->first) << " <- " << translate(s->first) << " x " << translate(r->first) << std::endl;
              gen_site_operator(mpos[i], l->first, l->second, s->first, r->first, r->second, oneint, twoint);
