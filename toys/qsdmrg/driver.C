                     ...
                     btas::DArray<2>& alphaIm)
{
  ...
  for(int i = 0; i < hreal.rows(); ++i) {
    cout << "\t";
    for(int j = 0; j < hreal.cols(); ++j) {
  ...
  for(int i = 0; i < himag.rows(); ++i) {
    cout << "\t";
    for(int j = 0; j < himag.cols(); ++j) {
               ...
               const btas::DArray<2>& d_subspace)
{
  ...
  for(int i = 0; i < pxm.rows(); ++i) {
    cout << "\t";
    for(int j = 0; j < pxm.cols(); ++j) {
  ...
  cout << "DEBUG: (S - D)(S + D)" << endl;
  for(int i = 0; i < mxp.rows(); ++i) {
    cout << "\t";
    for(int j = 0; j < mxp.cols(); ++j) {
