#ifndef __MPSXX_DMRG_DMRG_HPP
#define __MPSXX_DMRG_DMRG_HPP
...
/// Optimize 1-site wavefunctions (State-Averaged algorithm)
template<class Q>
double optimize_1site (
        bool forward,
  const std::vector<MPO  <double,Q>>& mpo0, ///< a set of MPO's locally stored.
        std::vector<BLOCK<double,Q>>& lopr, ///< a set of left  BLOCK operators locally stored.
        std::vector<BLOCK<double,Q>>& ropr, ///< a set of right BLOCK operators locally stored.
        std::vector<MPS  <double,Q>>& wfn0, ///< waves (current) for desired roots.
        std::vector<MPS  <double,Q>>& wfn1, ///< guess waves (next)
  const double& T,
  const int& M = 0)
{
  using std::cout;
  ...
  MPS<double,Q> diag(wfn0.q(),wfn0.qshape(),wfn0.dshape(),false);
  // loop over operators on local proc.
  for(size_t t = 0; t < mpo0; ++t) {
    compute_diagonal_elements(mpo0[t],lopr[t],ropr[t],diag);
  }
#ifndef _SERIAL
  // sum over procs.
  ...
  if(world.rank() == 0) {
    for(size_t p = 1; p < world.size(); ++p) {
      MPS<double,Q> tmp_recv;
      world.recv(p,p,tmp_recv);
      axpy(1.0,tmp_recv,diag);
    }
  }
  else {
    world.send(0,world.rank(),diag);
  }
  boost::mpi::broadcast(world,diag,0);
#endif

  cout << "\t\tFinished computing diagonal elements ( " << fixed << setprecision(2) << setw(8) << ts.lap() << " sec. ). " << endl;

  //
  // Diagonalize by Davidson eigensolver
  //

  davidson::Functor<3,Q> fsigma = boost::bind(compute_sigmavector<Q>,mpo0,lopr,ropr,_1,_2);
  std::vector<double> es = Davidson::diagonalize(fsigma,diag,wfn0);

  cout << "\t\tFinished optimizing wavefunctions... ( " << fixed << setprecision(2) << setw(8) << ts.lap() << " sec. ). " << endl;
...
#CURRENT
  ...
  if(forward) {
    ...
    cout << "\t\trenormalizing operators to the next..." << flush;
    btas::QSTArray<double,3,Q> ropr_tmp;
...
(bool forward, const btas::QSTArray<double,4,Q>& lmpo,
